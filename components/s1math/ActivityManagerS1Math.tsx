...
import { SUBJECTS_S1MATH, DOMAINS_S1MATH, SubjectS1Math } from '../../constants/s1math';
...
const ActivityManagerS1Math: React.FC<Props> = ({ activities, onAdd, onUpdate, onDelete, onSelect }) => {
  ...
  const filteredActivities = activities.filter(activity => {
    ...
    const matchesSubject = searchSubjects.length === 0 || searchSubjects.includes(activity.subject as SubjectS1Math);
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold flex items-center gap-2 text-slate-800">
          <List className="text-blue-600" /> Fiches Activités — 1ère Secondaire
        </h2>
        <button
      ...
      {isFormOpen && (
        <div className="bg-white p-8 rounded-2xl border shadow-xl">
          <h3 className="text-lg font-bold mb-6 flex items-center gap-2">
            <List className="text-blue-500" /> {editingId ? 'Modifier la fiche' : 'Créer une fiche pédagogique'}
          </h3>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={e => setFormData({...formData, subject: e.target.value as SubjectS1Math, domain: ''})}
              >
                {SUBJECTS_S1MATH.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}
              </select>
            </div>
            <div className="space-y-2">
              ...
              >
                ...
                {DOMAINS_S1MATH[formData.subject as SubjectS1Math]?.map(d => (
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between text-xs text-slate-400 px-1">
                <span>Facile</span><span>Moyen</span><span>Difficile</span>
              </div>
            </div>
            <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredActivities.map(activity => {
          const subjectInfo = SUBJECTS_S1MATH.find(s => s.value === activity.subject);
          return (
            <div
              key={activity.id}
            ...
            >
              ...
              <div className="p-5 flex-1">
                ...
                {activity.objective && (
                  <p className="text-slate-600 text-sm line-clamp-2 italic mb-4">"{activity.objective}"</p>
                )}
...
export default ActivityManagerS1Math;
