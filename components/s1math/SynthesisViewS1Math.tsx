...
import { SUBJECTS_S1MATH, SubjectS1Math, getGradeConfigS1 } from '../../constants/s1math';
...
const SynthesisViewS1Math: React.FC<Props> = ({ data }) => {
  const [selectedCycle, setSelectedCycle] = useState(1);
  const [searchTerm, setSearchTerm] = useState('');
  const [sortOption, setSortOption] = useState<SortOption>('alpha-asc');

  const getStudentAverage = (studentId: string, subject: SubjectS1Math) => {
  ...
  const sortedAndFilteredStudents = data.students
    ...
    .sort((a, b) => {
      if (sortOption === 'alpha-asc') return `${a.firstName} ${a.lastName}`.localeCompare(`${b.firstName} ${b.lastName}`);
      if (sortOption === 'alpha-desc') return `${b.firstName} ${b.lastName}`.localeCompare(`${a.firstName} ${a.lastName}`);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl border shadow-sm">
        <div>
          <h2 className="text-xl font-black flex items-center gap-2 text-slate-800">
            <BarChart3 className="text-blue-600" /> Pilotage 1ère Secondaire
          </h2>
          <p className="text-xs text-slate-500 font-bold uppercase tracking-wider">Moyennes par trimestre — Notes sur 20</p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-900 text-white text-[10px] uppercase tracking-widest font-black">
                ...
                {SUBJECTS_S1MATH.map(s => (
                  <th key={s.value} className="p-4 text-center whitespace-nowrap min-w-[120px]">
            ...
            <tbody className="divide-y text-sm">
              {sortedAndFilteredStudents.map(student => {
                ...
                const globalConfig = globalAvg !== null ? getGradeConfigS1(globalAvg) : null;
                return (
                  <tr key={student.id} className="hover:bg-blue-50/30 transition-colors group">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        {globalAvg !== null && (
                          <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-white text-[10px] font-black ${globalConfig?.color} w-fit`}>
                            <TrendingUp size={10} /> {globalAvg}/20
                          </div>
                        )}
                      </div>
                    </td>
                    ...
                    {SUBJECTS_S1MATH.map(s => {
                      ...
                      const config = avg !== null ? getGradeConfigS1(avg) : null;
                      return (
                        <td key={s.value} className="p-4 text-center">
                          {avg !== null ? (
                            <div className={`inline-flex items-center justify-center w-12 h-10 rounded-xl font-black text-xs text-white ${config?.color} shadow-sm`}>
                              {avg}/20
                            </div>
                          ) : (
...
export default SynthesisViewS1Math;
